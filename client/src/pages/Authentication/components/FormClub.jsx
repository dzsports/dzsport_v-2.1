...
const FormClub = (props) => {
        ...
        const Signup = async (event) => {
                ...
                if (!user.wilaya) {
                        errorss.push('wilayaError')
                } 
                if (!user.daira) {
                        errorss.push('dairaError')
                }
                if (!user.sport) {
                        errorss.push('sportError')
                } 
                setErrors(errorss);
                // console.log(errors);
                if (!errors.length) {
        ...
        <div className="part flex flex-col gap-4">
                ...
                <div className="input flex flex-col">
                        ...
                        <div className="places flex flex-col gap-2">
                                <div className="wilaya">
                                        ...
                                        <select name="wilaya" id="wilaya1" className={`wilayaSelect1 ${error.includes('wilayaError') ? 'red-border' : null} `} defaultValue={document.querySelectorAll('#wilaya1 option')[0]} onChange={handleChange}>
                                                
                                        </select>
                                </div>
                                <div className="daira1">
                                        <label htmlFor="daira" className=' ml-1' >الدائرة</label>
                                        <select name="daira" id="daira1" className={`dairaSelect1 ${error.includes('dairaError') ? 'red-border' : null} `} defaultValue={document.querySelectorAll('#daira1 option')[0]} onChange={handleChange}>
                                        </select>
                                </div>
                ...
                <div className="input flex flex-col">
                        ...
                        <input type="text" name="sport" id="sport1" onChange={handleChange} className={error.includes('sportError') ? 'red-border' : null} />
                </div>
        </div>
        </div>
