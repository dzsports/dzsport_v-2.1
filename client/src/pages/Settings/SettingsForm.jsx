...
const SettingsForm = () => {
        ...
        const handleSport = async (e) => {
                ...
                if (!user.sport) {
                        ...
                        setError('')
                        const formData = new FormData();
                        formData.append('sport',user.sport)
                        // HERE BACKEND CODE
                }
        }
        ...
        return (
                <>
                        ...
                        <form className=' bg-white rounded-lg px-3 py-4 text-center' onSubmit={handlePhone} >
                                ...
                                <input type="email" className={`w-full rounded-lg mt-3 bg-gray-100 ${error === 'phoneError' ? 'red-border': null}`} name="phone"  onChange={handleChange} placeholder='رقم الهاتف' />
